...
## 7. Search and logic specification
The portal implements a two-tier search mechanism in variant_v2.php and variants_v2.php.

** a. Gene Symbol Detection **
Queries matching the pattern:

^[A-Z0-9-]{2,20}$ are interpreted as candidate gene symbols.

For queries:
WHERE UPPER(TRIM(gene_symbol)) = :g is applied.

Additional post-query filtering tokenizes multi-gene fields to ensure exact gene matching. 
This design prevents unitended partial matches from LIKE queries

**b. Broad Keyword Search**
If the query does not resemble a gene symbol, a broader search is executed:
- gene_symbol LIKE :q_like
- OR genomic_variant LIKE :q_like
- OR studies LIKE :q_like

This allows discovery via:
-	Disease names
-	Study identifiers
-	HGVS notation
-	Genomic coordinates

## 8. Genome Coordinate Handling

The function:
- pick_genomic_variant_for_genome()
- selects the appropriate coordinate based on:
    -  User-selected genome build
	- Availability of paper-reported coordinates
    - Availability of lifted coordinates
The UCSC link is dynamically generated using:
ucsc_url_from_genomic_variant()

Mapping:
| Genome  | UCSC Database |
|---------|---------------|
| GRCh37  | hg19          |
| GRCh38  | hg38          |

This ensures reproducible coordinate navigation.

## 9.  Launching the Portal
...
## 12.  Data Provenance and Traceability
...
## 14.  Scientific Reproducibility Statement
