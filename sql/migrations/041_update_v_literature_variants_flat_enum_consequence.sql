-- 041_update_v_literature_variants_flat_enum_consequence.sql
USE autoimmune_db;
...
SELECT
  ...
  /* Variant type (structural class from alleles) */
  CASE
  ...
  /* Detailed consequence (ENUM value) */
  ldv.variant_impact                                          AS consequence_detail,

  /* Bucket consequence (derived from ENUM value) */
  CASE
    WHEN ldv.variant_impact IN ('frameshift','frameshift insertion','frameshift deletion','frameshift delins')
      THEN 'Frameshift'

    WHEN ldv.variant_impact IN ('inframe indel','inframe insertion','inframe deletion','inframe delins')
      THEN 'Inframe indel'
