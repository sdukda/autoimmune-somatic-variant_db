...
SELECT
  ...
  /* Raw consequence detail (exact stored value) */
  ldv.variant_impact AS consequence_detail,

  /* Normalised consequence bucket (UI-safe) */
  CASE
    WHEN ldv.variant_impact IS NULL
      THEN 'Unknown'

    WHEN ldv.variant_impact = 'unknown'
      THEN 'Unknown'

    WHEN ldv.variant_impact LIKE 'frameshift%'
      THEN 'Frameshift'

    WHEN ldv.variant_impact LIKE 'inframe%'
      THEN 'Inframe indel'

    WHEN ldv.variant_impact = 'frameshift'
      THEN 'Frameshift'

    WHEN ldv.variant_impact = 'inframe indel'
      THEN 'Inframe indel'

    WHEN ldv.variant_impact = 'missense'
      THEN 'Missense'

    WHEN ldv.variant_impact = 'nonsense'
      THEN 'Nonsense'

    WHEN ldv.variant_impact = 'splice'
      THEN 'Splice'

    WHEN ldv.variant_impact = 'synonymous'
      THEN 'Synonymous'

    WHEN ldv.variant_impact = 'noncoding'
      THEN 'Noncoding'

    WHEN ldv.variant_impact = 'intron'
      THEN 'Intron'

    WHEN ldv.variant_impact = 'utr'
      THEN 'UTR'

    WHEN ldv.variant_impact = 'promoter'
      THEN 'Promoter'

    WHEN ldv.variant_impact = 'intergenic'
      THEN 'Intergenic'
